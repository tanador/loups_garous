...
describe('hunter ability', () => {
  it('night kill triggers a shot', () => {
    const g = setup();
    applyDeaths(g, ['Hunter'], () => 'Wolf');
    expect(g.alive.has('Hunter')).toBe(false);
    expect(g.alive.has('Wolf')).toBe(false);
  });

  it('vote execution triggers a shot', () => {
    const g = setup();
    applyDeaths(g, ['Hunter'], () => 'Villager');
    expect(g.alive.has('Hunter')).toBe(false);
    expect(g.alive.has('Villager')).toBe(false);
    expect(g.alive.has('Wolf')).toBe(true);
  });

  it('two hunters dying resolve without loops', () => {
    ...
    g.players.forEach(p => (p.role = g.roles[p.id]));
    applyDeaths(g, ['H1', 'H2'], (hid) => (hid === 'H1' ? 'H2' : 'H1'));

...
export class Orchestrator {
  ...
  createGame(nickname: string, playerCount: number, wolves: number, socket: Socket) {
    const game = createGame({ maxPlayers: playerCount, wolves });
    const player: Player = addPlayer(game, { id: nickname, socketId: socket.id });
    this.store.put(game);
    this.bindPlayerToRooms(game, player, socket);
    this.emitLobbyUpdate();
    this.log(game.id, 'LOBBY', player.id, 'lobby.create', { players: playerCount, wolves });

...
export class Orchestrator {
    ...
    store = new GameStore();
    timers = new Map();
    rateCounters = new Map(); // per-socket simple limiter
    constructor(io) {
        this.io = io;
    ...
    createGame(nickname, variant, socket) {
        const v = variant === 'AUTO' ? (randomInt(2) === 0 ? 'V1' : 'V2') : variant;
        const game = createGame(v);
        const player = addPlayer(game, { id: randomUUID(), nickname, socketId: socket.id });
        this.store.put(game);
    ...
    joinGame(gameId, nickname, socket) {
    ...
    bindPlayerToRooms(game, player, socket) {
        ...
        if (game.roles[player.id]) {
            ...
            if (role === 'WOLF')
                socket.join(`room:${game.id}:wolves`);
            if (role === 'WITCH')
                socket.join(`room:${game.id}:witch`);
        }
    }
    emitLobbyUpdate() {
    ...
    tryAutostart(game) {
        if (game.players.length === 3 && game.state === 'LOBBY') {
            ...
            for (const p of game.players) {
                ...
                if (s)
                    this.bindPlayerToRooms(game, p, s);
            }
    ...
    // -------------------- Phases --------------------
    beginNightWolves(game) {
        if (!canTransition(game, game.state, 'NIGHT_WOLVES'))
            return;
        game.round += 1;
        game.night = {};
        game.wolvesChoices = {};
    ...
    wolvesChoose(gameId, playerId, targetId) {
        const game = this.mustGet(gameId);
        if (game.state !== 'NIGHT_WOLVES')
            throw new Error('bad_state');
        if (game.roles[playerId] !== 'WOLF')
            throw new Error('forbidden');
        if (!game.alive.has(targetId))
            throw new Error('invalid_target');
        if (game.roles[targetId] === 'WOLF')
            throw new Error('invalid_target');
    ...
    beginNightWitch(game) {
        if (!canTransition(game, game.state, 'NIGHT_WITCH'))
            return;
    ...
    witchDecision(gameId, playerId, save, poisonTargetId) {
        const game = this.mustGet(gameId);
        if (game.state !== 'NIGHT_WITCH')
            throw new Error('bad_state');
        if (game.roles[playerId] !== 'WITCH')
            throw new Error('forbidden');
        // save
        if (save) {
            if (!game.night.attacked)
                throw new Error('nothing_to_save');
            if (game.inventory.witch.healUsed)
                throw new Error('heal_already_used');
    ...
    beginMorning(game) {
        if (!canTransition(game, game.state, 'MORNING'))
            return;
    ...
    voteCast(gameId, playerId, targetId) {
        const game = this.mustGet(gameId);
        if (game.state !== 'VOTE')
            throw new Error('bad_state');
        if (!game.alive.has(playerId))
            throw new Error('dead_cannot_vote');
        if (targetId && !game.alive.has(targetId))
            throw new Error('invalid_target');
    ...
    // -------------------- Rate limit --------------------
    limit(socket, key, max = 10, windowMs = 5000) {

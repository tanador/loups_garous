<#
  build_and_run_6.ps1 - Multi-run helper (server + 6 clients)
  ...
  - Ouvre 6 instances du client, en mosaïque 3x2 sur l'écran choisi
  ...
  Comportement par défaut (sans -NoAuto):
    - 1ère instance:  _paramNick=fabrice_serveur, _autoCreate=true, _maxPlayers=6 (crée une partie 6 joueurs)
    ...
    - 6ème instance:  _paramNick=fabrice_6,      _autoCreate=false

  Exemples:
    .\build_and_run_6.ps1                          # écran principal, auto on
    .\build_and_run_6.ps1 -ScreenIndex 1           # écran secondaire
    .\build_and_run_6.ps1 -Port 3001               # serveur sur 3001
    .\build_and_run_6.ps1 -NoAuto                  # sans _paramNick/_autoCreate/_maxPlayers
...
try {
  Register-ObjectEvent -InputObject ([console]) -EventName CancelKeyPress -SourceIdentifier 'LG_CtrlC' -Action {
    ...
    try {
      ...
      if ($p) {
        try { Get-NetTCPConnection -LocalPort ([int]$p) -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { & taskkill /PID $_ /T /F 2>$null | Out-Null } } catch { }
        try { netstat -ano -p tcp | Select-String -SimpleMatch (":" + $p) | ForEach-Object { ($_ -split '\s+')[-1] } | Where-Object { $_ -match '^\d+$' } | Sort-Object -Unique | ForEach-Object { & taskkill /PID $_ /T /F 2>$null | Out-Null } } catch { }
      }
    } catch { }
    try { Get-CimInstance Win32_Process -Filter "Name='cmd.exe'" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'start_server\.cmd' } | ForEach-Object { & taskkill /PID $_.ProcessId /T /F 2>$null | Out-Null } } catch { }
    try { Get-Process -Name 'loup_garou_client' -ErrorAction SilentlyContinue | ForEach-Object { & taskkill /PID $_.Id /T /F 2>$null | Out-Null } } catch { }
  } | Out-Null
...
try { Get-CimInstance Win32_Process -Filter "Name='cmd.exe'" -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'start_server\.cmd' } | ForEach-Object { Stop-ProcessTree -Pid $_.ProcessId } } catch { }
try { Get-Process -Name cmd -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowTitle -like 'LG_SERVER_*' } | ForEach-Object { Stop-ProcessTree -Pid $_.Id } } catch { }
...
$serverTitle = "LG_SERVER_$Port"; $cmdInner = "title $serverTitle & `"$serverCmd`" $Port"
...
if (-not (Test-Path $candidate)) {
  $exe = Get-ChildItem -Path (Join-Path $repoRoot 'build\windows') -Recurse -Filter '*.exe' -ErrorAction SilentlyContinue | Where-Object { $_.FullName -match '\\runner\\(Release|Debug)\\' } | Select-Object -First 1
...
$positions = @(
  ...
  @{ X = $baseX + ($tileW * 2); Y = $baseY + ($tileH * 1) }
)
...
$procs = @(); $p=0
$procs += Spawn-Client -Nick 'fabrice_serveur' -AutoCreate $true  -AutoMaxPlayers 6 -X $positions[$p].X -Y $positions[$p].Y -W $tileW -H $tileH -NoAuto:$NoAuto; $p++
...
$procs += Spawn-Client -Nick 'fabrice_5'      -AutoCreate $false -AutoMaxPlayers 0 -X $positions[$p].X -Y $positions[$p].Y -W $tileW -H $tileH -NoAuto:$NoAuto; $p++
$procs += Spawn-Client -Nick 'fabrice_6'      -AutoCreate $false -AutoMaxPlayers 0 -X $positions[$p].X -Y $positions[$p].Y -W $tileW -H $tileH -NoAuto:$NoAuto

for ($i=0; $i -lt $procs.Count; $i++) { Set-WindowBounds -Process $procs[$i] -X $positions[$i].X -Y $positions[$i].Y -W $tileW -H $tileH }

Write-Host "[OK] Server and 6 clients launched."
